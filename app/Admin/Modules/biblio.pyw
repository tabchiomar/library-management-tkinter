...
def chercher():
    ...
    cursor = conn.cursor()
    query="SELECT * FROM biblio WHERE nom='"+name+"' OR prenom='"+name+"' OR idbiblio='"+name+"'"
    ...
    id_user=result[7]
    app.scrollable_label_button_frame.add_item(f"{id_user} | Bibliothécaire : {idadherent} | {nom_adherent} | {prenom_adherent} | {CIN_adherent} | {datead}", image=app.adherent_image)
    conn.close()
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.title("Gestion des bibliothécaires - Administrateur système")
        ...
        self.navigation_frame_label = customtkinter.CTkLabel(self.navigation_frame, text="  Gestion des biblios", 
    ...
    def select_frame_by_name(self, name):
        ...
        self.entry = customtkinter.CTkEntry(self, placeholder_text='Chercher par nom ou prenom de bibliothécaire', width=650)
        ...
        for row in result:
            ...
            id_user=result[i][7]
            i=i+1
            self.scrollable_label_button_frame.add_item(f"{id_user} | Bibliothécaire: {idadherent} | {nom_adherent} | {prenom_adherent} | {CIN_adherent} | {datead}", image=self.adherent_image)
        conn.close()
        # create second frame----------------------------------------------------------------------------------------------------------------
        def insert():
            ...
            if(nom=='' or prenom=='' or cin=='' or mail=='' or datenaissance=='' or username=='' or password==''):
                ...
                if(numrows==0):
                    cursor=conn.cursor()
                    sql="INSERT INTO Users VALUES(NULL, ?,? ,'bibliothecaire')"
                    cursor.execute(sql,(username,password))
                    id=cursor.lastrowid
                    cursor.close()
                    cursor2=conn.cursor()
                    sql2="INSERT INTO Biblio VALUES(NULL, ?, ?, ?, ?, ?, ?, ?)"
                    cursor2.execute(sql2,(cin, nom, dateinscription, prenom, mail, datenaissance, id))
                    conn.commit()
                    print('done')
                    CTkMessagebox(title='Erreur',message='Bibliothécaire ajouté!', icon="warning")
                else:
                    CTkMessagebox(title='Erreur',message="Nom d'utilsiateur existant", icon="warning")
  

        self.add_frame = customtkinter.CTkFrame(self, corner_radius=0, fg_color="transparent")
        labelA = customtkinter.CTkLabel(self.add_frame, text="Ajout bibliothécaire:", fg_color="transparent")
        labelA.place(x=30,y=30)
        labelnom_adherent = customtkinter.CTkLabel(self.add_frame, text="Nom du bibliothécaire :", fg_color="transparent")
        labelprenom_adherent = customtkinter.CTkLabel(self.add_frame, text="Prenom du bibliothécaire:", fg_color="transparent")
        labelCIN_adherent = customtkinter.CTkLabel(self.add_frame, text="CIN du bibliothécaire:", fg_color="transparent")
        labelemail_adherent = customtkinter.CTkLabel(self.add_frame, text="Mail du bibliothécaire:", fg_color="transparent")
        ...
        entrynom_adherent.place(x=230,y=70)
        entryprenom_adherent.place(x=230,y=110)
        entryCIN_adherent.place(x=230,y=150)
        entryemail_adherent.place(x=230,y=190)
        entrydatenaissance_adherent.place(x=230,y=230)
        entryusername_adherent.place(x=230,y=310)
        entrypassword_adherent.place(x=230,y=350)

        submit = customtkinter.CTkButton(self.add_frame, text= "Enregistrer bibliothécaire" , command=insert)
        ...
        labelnom_haut = customtkinter.CTkLabel(self.edit_frame, text="Username du bibliothécaire:", fg_color="transparent") #label
        ...
        entrynom_haut.place(x=220, y=50)

        
        def remplirchamps():
            conn=sqlite3.connect(db_path)
            check_entry=entrynom_haut.get()
            sql2="select id, password from users U, biblio B where username='"+str(check_entry)+"' and U.id=B.iduser"
            cursor1=conn.cursor()
            ...
            if(check_entry=='' ):
                ...
                cursor=conn.cursor()
                sql="SELECT * FROM biblio WHERE iduser='"+str(resultat[0])+"'"
                cursor.execute(sql)
                result=cursor.fetchone()
        ...
        def update_adherent():
            ...
            if(nom=='' or prenom=='' or cin=='' or email=='' or naissance=='' or passw==''):
                ...
                sql="UPDATE Biblio SET nom=?, prenom=?, CIN=?, email=?, datenaissance=? where iduser=?"
                cursor=conn.cursor()
        ...
        label_delete_name = customtkinter.CTkLabel(self.delete_frame, text="Username du biblio", fg_color="transparent")
        ...
        def delete():
            ...
            sql2="DELETE FROM Biblio WHERE iduser='"+str(id)+"'"
            ...
            if(count!=0):
                ...
                CTkMessagebox(title='Erreur',message="Aucun bibliothécaire de ce nom/id", icon="warning")
...
# show selected frame
    ...
    def label_button_frame_event(self, item):
        ...
        sql="DELETE from Biblio where iduser='"+id+"'"
        cursor.execute(sql)
